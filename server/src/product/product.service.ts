...
export class ProductService {
    ...
    async getTotalProductsCount() {
        const productCount = await this.db.select({ count: count() }).from(product).where(isNull(product.deletedAt));
        return productCount[0].count;

    }

    async restoreProduct(id: string) {
        const restoredProduct = await this.db.update(product).set({ deletedAt: null }).where(eq(product.id, id)).returning()
        return restoredProduct[0];

    }
    async softDeleteProduct(id: string) {

        const deletedProduct = await this.db.update(product).set({ deletedAt: new Date() }).where(eq(product.id, id)).returning();
